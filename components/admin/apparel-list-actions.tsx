...
/**
 * Edit / Delete actions for apparel row.
 */
import Link from "next/link";
import { Pencil, Trash2 } from "lucide-react";
import { useState } from "react";

interface ApparelListActionsProps {
  slug: string;
}

export function ApparelListActions({ slug }: ApparelListActionsProps) {
  const [isDeleting, setIsDeleting] = useState(false);

  const handleDelete = async () => {
    if (!confirm("Delete this apparel product? This cannot be undone.")) return;
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/admin/apparel/${slug}`, {
        method: "DELETE",
      });
      if (res.ok) {
