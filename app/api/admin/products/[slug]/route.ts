...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const specFields = ["voltage", "capacity", "batteryType", "warranty", "connectorType"];
    const specUpdate: Record<string, unknown> = {};
    for (const f of specFields) {
      if (body[f] != null) {
        specUpdate[f] = typeof body[f] === "number" ? body[f] : String(body[f]).trim();
      }
    }
    if (Object.keys(specUpdate).length > 0) {
      await BatterySpecification.updateOne(
        { productId: product._id },
        { $set: specUpdate }
      );
    }

    const updated = await Product.findOne({ slug }).lean();
...
export async function DELETE(_request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    await BatterySpecification.deleteOne({ productId: product._id });
